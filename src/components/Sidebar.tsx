...
import { FolderOpen, LayoutDashboard, Database, ChevronRight, ArrowLeft, Plus } from 'lucide-react';
  ...
  return (
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      <div className="sidebar-projects">
        {projects.map(p => (
          <div key={p.id}>
            <button className={`sidebar-project ${activeProject?.id === p.id && view === 'workspace' ? 'active' : ''}`} onClick={() => onSelectProject(p)} title={p.name}>
        ...
        <button className="sidebar-new-project" onClick={onNewProject} title="New Project">
          <div className="new-proj-btn-inner">
            <div className="new-proj-icon-box"><Plus size={16}/></div>
